...
# Copy download.properties

FILE="${GEOSERVER_DATA_DIR}/download.properties"
# If file doesn't exist OR contains "# Max #of features"
if [ ! -f "$FILE" ] || grep -q "# Max #of features" "$FILE"; then
    echo "download.properties is missing or using default, override default..."
    cp /download.properties "$FILE"
else
    echo "Valid download.properties already exists, skipping copy."
fi
